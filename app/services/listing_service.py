...
from app.schemas.listing import ListingCreate, ListingUpdate, AllergenFilterRequest, AllergenFilterResponse
...
async def create_listing(data: ListingCreate, vendor: Vendor, session: AsyncSession) -> Listing:
    ...
    for allergen_code in data.allergens:
        la = ListingAllergen(listing_id=listing.id, allergen_code=allergen_code)
        session.add(la)
...
async def get_listings(
    ...
    exclude_allergens: Optional[List[AllergenCode]] = None,
    ...
    query = select(Listing).where(
        ...
        Listing.pickup_window_end > datetime.utcnow(),
    )
    ...
    if lat is not None and lng is not None:
        listings = [l for l in listings if _haversine(lat, lng, l.latitude, l.longitude) <= 10]
